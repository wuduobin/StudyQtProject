...
{
    vm_size_t pageSize;
    vm_statistics64_data_t vmStatus;

    mach_port_t machPort = mach_host_self(); //A machPort is a kind of special connection to the kernel that enables us to request information about the system.
    mach_msg_type_number_t count = sizeof(vmStatus) / sizeof(natural_t); //natural_t == unsigned int
    host_page_size(machPort, &pageSize);

    host_statistics64(machPort, HOST_VM_INFO, (host_info64_t)&vmStatus, &count); //retrieve virtual memory statistics

    qulonglong freeMemory = (int64_t)vmStatus.free_count * (int64_t)pageSize;

    qulonglong totalMemoryUsed = ((int64_t)vmStatus.active_count + (int64_t)vmStatus.inactive_count + (int64_t)vmStatus.wire_count)
                                    * (int64_t)pageSize;
...
QVector<qulonglong> SysInfoMacImpl::cpuRawData()
{
    ...
    for (unsigned int i = 0; i < cpuCount; i++) {
        unsigned int maxTicks = CPU_STATE_IDLE * i;
